...
ABlasterCharacter::ABlasterCharacter()
{
	...
	FollowCamera->bUsePawnControlRotation = false;//因为相机附加在弹簧臂上
	//角色不再死板地跟随控制器的朝向， 控制器的朝向主要用来控制 摄像机视角，而不是角色身体
...
void ABlasterCharacter::PostInitializeComponents()
{
	...
	if (Combat)
	{
		Combat->Character = this;//所以combatcomponent.h中Character 指针变量就是这和this，
	}
	if (Buff)
	{
		Buff->Character = this;
...
void ABlasterCharacter::PlayFireMontage(bool bAiming)
{
	...
	//需要用到动画实例  处理动画 还没有开枪产生子弹  在WeaponMesh->PlayAnimation(FireAnimation, false);看见开火特效
...
{
	AO_Pitch = GetBaseAimRotation().Pitch;//注意
