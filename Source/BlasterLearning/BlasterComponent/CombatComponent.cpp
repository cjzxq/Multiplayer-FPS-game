...
void UCombatComponent::BeginPlay()
{
	...
	if (Character)
	{
		//GetCharacterMovement()会得到一个 
		// UCharacterMovementComponent* 类型的CharacterMovement
		// FORCEINLINE UCharacterMovementComponent* GetCharacterMovement() const { return CharacterMovement; }
		//因为CharacterMovement是Character类中的私有变量，而MaxWalkSpeed这个变量在CharacterMovementComponent.h中
...
void UCombatComponent::FireProjectileWeapon()
{
	if (EquippedWeapon && Character)
	{
		...
		ServerFire(HitTarget, EquippedWeapon->FireDelay);//客户端调用
	}
}
...
//服务器广播 这个RPC会由服务器发往所有正在观看这个游戏的客户端（包括服务器自己）（包括最初开枪的那个客户端）。
void UCombatComponent::MulticastFire_Implementation(const FVector_NetQuantize& TraceHitTarget)
{
	//Character->IsLocallyControlled()判断这个角色是否由本机玩家控制。 !Character->HasAuthority()：判断本机是否不是权威服务器（即我是一个客户端）。
...
{
	FVector2D ViewportSize;//视口尺寸大小
	if (GEngine && GEngine->GameViewport)
	{
		//相当于void GetViewportSize(FVector2D& OutViewportSize) const;，引用传递。
		GEngine->GameViewport->GetViewportSize(ViewportSize);//引擎拿到当前游戏视口大小（用于计算屏幕中心）。
	}

	FVector2D CrosshairLocation(ViewportSize.X / 2.f, ViewportSize.Y / 2.f);//屏幕中间的x，y坐标
	...
	if (bScreenToWorld)
	{
		...
		{
			TraceHitResult.ImpactPoint = FVector_NetQuantize(End);//TraceHitResult.ImpactPoint命中结果
		}
	}
}
