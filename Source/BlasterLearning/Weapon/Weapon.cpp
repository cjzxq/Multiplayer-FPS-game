...
AWeapon::AWeapon()
{
	...
	//就是给武器加了一个 静态网格（外观模型），并把它设成根组件。创建指针WeaponMesh对象
...
void AWeapon::GetLifetimeReplicatedProps(TArray<FLifetimeProperty>& OutLifetimeProps) const
{
	...
	DOREPLIFETIME(AWeapon, WeaponState);//告诉引擎，同步WeaponState这个变量
	DOREPLIFETIME_CONDITION(AWeapon, bUseServerSideRewind, COND_OwnerOnly);
}

void AWeapon::OnSphereOverlap(UPrimitiveComponent* OverlappedComponent, AActor* OtherActor, UPrimitiveComponent* OtherComp, int32 OtherBodyIndex, bool bFromSweep, const FHitResult& SweepResult)
{
	//判断进入区域的 Actor 是否是玩家角色	如果不是玩家 → 不处理
	ABlasterCharacter* BlasterCharacter = Cast<ABlasterCharacter>(OtherActor);
	if (BlasterCharacter)
	{
		//如果武器是旗帜（Flag），并且玩家属于同队 → 不处理（防止捡到自己队的旗）
...
void AWeapon::Fire(const FVector& HitTarget)
{
	...
	{
		//
		WeaponMesh->PlayAnimation(FireAnimation, false);//WeaponMesh：这是一个 USkeletalMeshComponent
	}
	if (CasingClass)
	{
