...
void AProjectileWeapon::Fire(const FVector& HitTarget)
{
	...
	APawn* InstigatorPawn = Cast<APawn>(GetOwner());
	 const USkeletalMeshSocket* MuzzleFlashSocket = GetWeaponMesh()->GetSocketByName(FName("MuzzleFlash"));//获取枪口插槽
	 UWorld* World = GetWorld();
	 if (MuzzleFlashSocket && World)
	 {
		 //“查询 GetWeaponMesh() 所指向的武器网格体上，名为 MuzzleFlashSocket 的插槽， 
		 // 计算出它当前在游戏世界中的精确位置和朝向，并将结果（位置、旋转、缩放）存储到 SocketTransform 变量中。”“
		 // 以后所有枪口火焰特效、子弹生成点，都应该放在这个精确的位置上。”
